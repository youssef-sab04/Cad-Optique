...
const SalesOrderTable = ({ salesOrders, onAddItem, onDelete, onDeleteOI, onConfirm, onReceipt }) => {
    ...
    const toggleExpand = (id) => {
        ...
        setExpandedId(id);
        setSOI(null);
        dispatch(fetchSalesOrderItemsID(toast, id, setBtnLoader, setSOI));
    };
                        ...
                        <React.Fragment key={order.id}>
                            ...
                            >
                                ...
                                <td className="px-4 py-3 text-sm text-slate-800 font-medium">
                                    {order.clientDTO?.nom} {order.clientDTO?.prenom}
                                </td>
                                <td className="px-4 py-3">
                                    <span className={`text-xs font-medium px-2 py-1 rounded-md ${statusStyles[order.status] || "bg-slate-100 text-slate-600"}`}>
                                ...
                                <td className="px-4 py-3 text-sm text-slate-800 font-semibold text-right">
                                    {Number(order.totalprice ?? 0).toFixed(2)} DH
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex justify-end items-center gap-3">
                                        ...
                                        <button
                                            onClick={(e) => { e.stopPropagation(); onReceipt(order.id); }}
